...
function [ MFCCs, FBEs, frames ] = MFCC(speech, fs)
    % Clean-up MATLAB's environment
    clear all; close all; clc;  

    [speechnew, fs] = transfer(speech, fs);
    % Define variables
   ...
   % [speech2, fs2] = audioread(mp3_file2);



    % Feature extraction (feature vectors as columns)
    [ MFCCs, FBEs, frames ] = ...
                    mfcc( speechnew, fs, Tw, Ts, alpha, hamming, [LF HF], M, C+1, L );
...
%             title( 'Mel frequency cepstrum' );
    ...
    time = [ 0:length(speechnew)-1 ]/fs;           % time vector (s) for signal samples 
